...
<?php
	...
	for (;;) {
		...
		$keyWord=md5($ticket."19950326");
		
		$historySQL=mysql_query("SELECT * FROM `loginList` WHERE `key`='".$keyWord."'");
		$history=mysql_fetch_array($historySQL,MYSQL_NUM);
		if (count($history)>1) continue;
		
		$historySQL=mysql_query("SELECT * FROM `loginList` WHERE `user`='".$user."'");
		$history=mysql_fetch_array($historySQL,MYSQL_ASSOC);
		if (count($history)>1) {
			mysql_query("UPDATE `loginList` SET `key`='".$keyWord."',`time`=NOW() WHERE `user`='".$user."';");
		} else {
			mysql_query("INSERT INTO `loginList`(`key`,`user`,`time`) VALUES('".$keyWord."','".$user."',NOW());");
		}
	...
	$ticketL=$userData["ticketL"];
	mysql_query("UPDATE `userData` SET `ticketL`='".$ticket."' WHERE `user`='".$user."';");
	
	$dataList=array("studentID","place","post","name","nickname","sex");
	$dataListN=count($dataList);
	$dataStr="";
	for ($i=0;$i<$dataListN;$i++) {
		$dataStr.=" ".$dataList[$i].'="'.$userData[$dataList[$i]].'"';
	}
	...
	echo "<script> location.href='http://127.0.0.1:88/CFASWebsite/loginData.php?user=".$user."&data=".$dataStr."&ticketL=".$ticketL."&ticket=".$ticket."' </script>";
