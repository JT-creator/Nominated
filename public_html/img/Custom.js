	...
	ASSourceNameList=[
		...
		{name:"startScene",src:"Image/Scene/Origin/main.jpg"},
	...
	ASSourceLoadList=[
		{src:"Data/data0.xml"},
		{name:"loadingScene",src:"Image/Scene/main.jpg"},
		{src:"Image/logo.png"},
		{src:"Image/logoL.png"},
	...
	function BasicLoading(buildP) {
		...
		target.cover=ASElement({
			...
			style:{
				...
				width:"100%",
				height:"100%",
				animation:"basicLoadingRotate 2s linear infinite",
			},
		});
	...
	function BasicInput(buildP) {
		...
		if (buildP.model=="excoList") {
			buildP1.type="radioPoint";
			buildP1.block=[];
			var data=ASGetSourceByName("data0").getNodeByName("exco13");
	...
	function buildScene() {
		...
		SCENE.currentPic="main.jpg";
			SCENE.pic0=ASElement({
				nodeName:"img",
				src:"Image/Scene/main.jpg",
				onload:function() {
			...
			SCENE.copyright=ASElement({
				nodeName:"p",
				innerHTML:"©2017-2018 Chinese Folk Art Society, HKUSTSU. All Right Reserved.",
				style:{
					position:"absolute",
	...
	function buildPage_FunctionL() {
		...
		for (var o=0;o<data.node.length;o++) {
			...
			PAGE[data.node[o].nodeName+"List"].updateList=function() {
				...
				var dataE=ASGetSourceByName("data0").getNodeByName("exco13");
				this.list.clearRow();
	...
	function buildPage_Exco13() {
		
		PAGE["exco13"]=ASElement({
			nodeName:"div",
			style:{
				display:"block",
				width:DEFAULT_PAGE_WIDTH,
	...
	function buildPageV_Exco13() {
		
		PAGE_V["exco13"]=ASElement({
			nodeName:"div",
			wCenter:true,
			style:{
				width:VERTICAL_PAGE_WIDTHL,
	...
	function buildPage_ExcoB() {
		...
		var pageName=["exco12","exco11","exco10","exco09"];
	...
	function buildPage_Document() {
        ...
        var dataName = ["Document/Constitution.pdf", "",""];
		...
		for (var o=0;o<pageName.length;o++) {
			...
			PAGE_V[pageName[o]].build=function() {
				...
				this.pageC=ASElement({
					nodeName:"img",
					src:"Document/"+this.dataName+"/"+this.dataName+"-"+"001"+".png",
					style:{
						display:"block",
						marginLeft:"auto",
						marginRight:"auto",
	...
	function Shell(buildP) {
		...
		target.page.root=target;
			...
			target.slide=BasicScroll({
				...
				heightA:ASWindowHeight-DEFAULT_SHELL_HEAD_HEIGHT,
				style:{
					position:"absolute",
					top:DEFAULT_SHELL_HEAD_HEIGHT,
					left:(ASWindowWidth-target.widthA)/2,
	...
	function Card(buildP) {
		...
		if (buildP.titleC || buildP.titleE) {
			...
			target.appendChild(target.bottom);
				...
				var w=target.widthA,h=target.heightA,h2=target.titleHeight,s=target.edge;
				target.bottomW=ASElement({
					nodeName:"canvas",
					width:target.widthA,
					height:target.heightA,
					style:{
				...
				target.bottom.appendChild(target.bottomW);
				target.bottomW.draw({
					type:"polygon",
					polygon:[
						0,0,
					...
					colorE:[
						"linear",0,0,Math.max(target.widthA,target.heightA),Math.max(target.widthA,target.heightA),
						0,"rgba(255,255,255,0.8)",
						0.5,"rgba(255,255,255,0.6)",
						0.8,"rgba(225,225,225,0.3)",
						1,"rgba(225,225,225,0.1)"
					],
				});
				target.bottomR=ASElement({
					nodeName:"canvas",
					width:target.widthA+2,
					height:target.heightA+2,
					style:{
						position:"relative",
						top:-target.heightA-1+target.heightA%2,
						borderRadius:target.edge,
						transformOrigin:"50% 50%",
						transform:{
							translateY:-(target.titleHeight-target.edge)/2*(target.heightA/(target.heightA-target.titleHeight-target.edge)),
							scaleX:target.widthA/(target.widthA-target.edge*2),
							scaleY:target.heightA/(target.heightA-target.titleHeight-target.edge),
						},
					},
				});
				target.bottom.appendChild(target.bottomR);
				target.bottomR.y0=-(target.titleHeight-target.edge)/2*(target.heightA/(target.heightA-target.titleHeight-target.edge));
				target.bottomR.scaleX0=target.widthA/(target.widthA-target.edge*2);
				target.bottomR.scaleY0=target.heightA/(target.heightA-target.titleHeight-target.edge);
				target.bottomR.draw({
					type:"polygon",
					polygon:[
						0,0,
				...
				target.bottomC=ASElement({
					nodeName:"div",
					style:{
						position:"relative",
						top:-target.heightA*2-2,
						width:target.widthA,
						height:target.heightA,
						borderRadius:target.edge,
						opacity:0,
						boxShadow:"8px 8px 15px rgba(0,0,0,0.8) inset",
					},
				});
				target.bottom.appendChild(target.bottomC);
